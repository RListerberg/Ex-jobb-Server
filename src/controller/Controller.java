...
/**
 * Created by robin on 2017-01-30.
 */
public class Controller {
	...
	public void createQuestion() {
		...
		question.setQuestion("Iron Fist");
	...
	public void showQuestWithCategory(){
		em.getTransaction().begin();
	...
	public void getCorrectAnswer(){
		em.getTransaction().begin();
		Query getCorrectAnswer = em.createQuery("SELECT c FROM CategoriesEntity c WHERE c.category ='Marvel'");

		CategoriesEntity categorResult = (CategoriesEntity) getCorrectAnswer.getSingleResult();
		List<QuestionsEntity> questTempList = new ArrayList<>();

		for( QuestionsEntity q : categorResult.getQuestions()){
			questTempList.add(q);

			for (int i = 0; i < 3; i++) {
				System.out.println(questTempList.get(questTempList.size() - 1).getAnswersList().get(i).isCorrect());
			}
		}
		em.getTransaction().commit();
	}
